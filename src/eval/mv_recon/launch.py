...
def get_args_parser():
    ...
    parser.add_argument("--simple_time", action="store_true", help="run simple wall-clock timing")
    parser.add_argument("--profile", action="store_true", help="run full profiling with FLOPs and memory")
    return parser
...
def main(args):
    ...
    with torch.no_grad():
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    if model_name == "stream3r" or "VGGT":
                        ...
                        with torch.cuda.amp.autocast(dtype=dtype):
                            with torch.no_grad():
                                if args.simple_time or not args.profile:
                                    ...
                                    torch.cuda.synchronize()
                                    t1 = time.time()
                                    print(f"Inference time: {(t1 - t0)*1000.0:.1f} ms")
                                else:
                                    # Full profiling mode
                                    ...
                                    torch.cuda.synchronize()
                                    t0 = time.time()
                                    with profiler.profile(use_cuda=True, profile_memory=False, with_flops=True) as prof:
                                        results = model.inference(batch)
                                    torch.cuda.synchronize()
                                    t1 = time.time()
                                    ...
                                    print(f"FLOPs: {total_flops/1e9:.2f} GFLOPs, Peak GPU mem: {peak/1024**2:.1f} MB")
                    ...
                    # --- HTML visualization of aligned point cloud ---
                    # Use the just-written `pcd` object for visualization
                    pts_np = np.asarray(pcd.points)
                    colors_np = np.asarray(pcd.colors)
                    # Ensure colors are in 0-1, convert to 0-255 for Plotly
                    colors_uint8 = (colors_np * 255).clip(0, 255).astype(np.uint8)
                    # Compose Plotly color strings
                    color_strings = [
                        f"rgb({r},{g},{b})"
                    ...
                    fig.update_layout(
                        ...
                        title=f"Aligned Point Cloud: {scene_id}"
                    )
                    html_path = os.path.join(
                        save_path, f"{scene_id.replace('/', '_')}-mask_align.html"
                    )
        ...
        # After all batches for this dataset, print the forward call count once.
            if accelerator.is_main_process:
                print(f"Total model.forward() calls: {model._forward_calls}")
